      ...
      if (res.status === 201) {
        ...
        if (res.status === 409) {
          ...
          const firstError = fieldErrors
            ...
            : null
          setError(firstError ?? 'Invalid input. Please check your details.')
        } else {
        ...
        <Card className="border border-[#e5e5e5] rounded-[14px] shadow-[oklab(0.145_-0.00000143796_0.00000340492_/_0.1)_0px_0px_0px_1px] bg-white">
          <CardHeader className="px-4 pt-4 pb-2">
            ...
            <CardDescription className="text-[14px] text-[#737373]">
              Sign up to start running performance tests
            </CardDescription>
          </CardHeader>
          <CardContent className="px-4 pb-4">
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div className="flex flex-col gap-2">
                <Label htmlFor="name" className="text-[14px] font-medium text-[#0a0a0a]">
                  Name <span className="text-[#737373] font-normal">(optional)</span>
                </Label>
                <Input
                  id="name"
                  type="text"
              ...
              <p className="text-center text-[13px] text-[#737373]">
                ...
                <Link href="/sign-in" className="text-[#0a0a0a] font-medium hover:underline">
                  Sign in
                </Link>
              </p>
            </form>
